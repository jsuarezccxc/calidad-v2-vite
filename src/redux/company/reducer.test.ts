import { ICompanyState, reducer as userReducer } from './reducer';
import {
    setCompany,
    setDocumentTypes,
    setCountries,
...
test('It should change state to information company', () => {
    const action = setCompany(information);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, information: information });
});

test('It should change state to document types', () => {
    const action = setDocumentTypes(document_types);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, document_types: document_types });
});

test('It should change state to countries', () => {
    const action = setCountries(countries);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, countries: countries });
});

test('It should change state to departments', () => {
    const action = setDepartments(departments);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, departments: departments });
});

test('It should change state to cities', () => {
    const action = setCities(cities);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, cities: cities });
});

test('It should change state to departments for country', () => {
    const action = setCountryDepartments(departments);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, country_departments: departments });
});

test('It should change state to cities for department', () => {
    const action = setDepartmentCities(cities);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, department_cities: cities });
});

test('It should change state to cities for ciuu', () => {
    const action = setCiius(ciius);
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, ciius: ciius });
});

test('It should change state to responsibilities', () => {
...
test('IT should change state error and save it in company redux', () => {
    const action = setError('Fallo el servidor');
    const state = userReducer(initialState, action);
    expect(state).toEqual({ ...state, error: 'Fallo el servidor' });
});
